  ...
  return (
    <div className={cn("relative w-full text-primary", className)} style={{ height }}>
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" aria-hidden>
        <defs>
          <linearGradient id="crm-area-grad" x1="0" y1="0" x2="0" y2="1">
        ...
        <path d={areaPath} fill="url(#crm-area-grad)" />
        <motion.path
          d={linePath}
          fill="none"
          stroke="currentColor"
    ...
    >
      ...
      {(centerValue || centerLabel) && (
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          {centerValue && (
            <span className="font-[family-name:var(--font-display)] text-xl font-bold">{centerValue}</span>
          )}
