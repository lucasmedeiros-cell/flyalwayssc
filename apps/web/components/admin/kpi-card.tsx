...
import type { AdminKpi } from "@vialta/types";
import { cn, formatCompact, formatMoneyCompact } from "@/lib/utils";

export function KpiCard({ kpi, currency }: { kpi: AdminKpi; currency: string }) {
  const value =
    kpi.unit === "money"
      ? formatMoneyCompact(kpi.value, currency)
      : kpi.unit === "pct"
        ? `${kpi.value}%`
        : formatCompact(kpi.value);

  const up = kpi.deltaPct >= 0;
  // "Bueno" = sube y no está invertido, o baja y está invertido (cancelaciones).
